...
from action_potential.fft import FFT
...
class FFTExperiment(Experiment):
    def run(self):
        ...
        for network, ax in zip(networks, axs):
            ...
            _, e_voltage, _ = network.simulate(T)[2]
            FFT.plot(e_voltage, ax, "Excitatory neurons")
            ax.set_title(network.name)
            ax.legend()

        fig.tight_layout()
        fig.savefig(os.path.join(self.output_dir, "FFTExperiment.png"))
